...
class DashboardServer {
  ...
  setupPublicAPI() {
    ...
    this.app.get("/api/v1/newsletter/unsubscribe", async (req, res) => {
      try {
        ...
        res.send(`
          ...
          <html>
          <head>
            <title>Unsubscribed - Nexus Newsletter</title>
            <style>
              body { font-family: Arial, sans-serif; background: #1a202c; color: white; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }
              .container { text-align: center; padding: 40px; background: rgba(102, 126, 234, 0.1); border-radius: 15px; border: 2px solid #667eea; }
              h1 { color: #667eea; }
          ...
          <body>
            <div class="container">
              ...
              <p>You've been removed from the Nexus newsletter.</p>
              <p>Sorry to see you go! 💔</p>
              <br>
              <a href="https://azzraya.github.io/Nexus/">← Back to Nexus</a>
            </div>
          </body>
          </html>
        `);
        ...
        res.status(400).send(`
          ...
          <html>
          <head>
            <title>Error - Nexus Newsletter</title>
            <style>
              body { font-family: Arial, sans-serif; background: #1a202c; color: white; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }
              .container { text-align: center; padding: 40px; background: rgba(255, 0, 0, 0.1); border-radius: 15px; border: 2px solid #ff0000; }
              h1 { color: #ff0000; }
            </style>
          </head>
          <body>
    ...
    this.app.get("/api/admin/newsletter/stats", async (req, res) => {
      try {
    ...
    this.app.get("/api/admin/newsletter/export", async (req, res) => {
      try {
        // Check admin password
        const adminPassword = req.headers["x-admin-password"];
        if (adminPassword !== process.env.ADMIN_PASSWORD) {
