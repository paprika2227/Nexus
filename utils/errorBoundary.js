...
class ErrorBoundary {
  ...
  setupRecoveryStrategies() {
    ...
    this.recoveryStrategies.set('SQLITE_', async (error, context) => {
      logger.warn('[ErrorBoundary] Database error detected, attempting recovery', { error: error.message });
      ...
      try {
        ...
        if (error.code === 'SQLITE_CONSTRAINT') {
          logger.info('[ErrorBoundary] Constraint violation handled gracefully');
    ...
    this.recoveryStrategies.set('DiscordAPI', async (error, context) => {
      logger.warn('[ErrorBoundary] Discord API error detected', { error: error.message });
      ...
      try {
        ...
        if (error.code === 50013) { // Missing Permissions
          return { 
            recovered: true, 
            retry: false,
            userMessage: '❌ I don\'t have permission to do that! Please check my role permissions.'
          };
        }
        ...
        logger.error('[ErrorBoundary] Discord API recovery failed', recoveryError);
      }
    ...
    this.recoveryStrategies.set('Network', async (error, context) => {
      if (error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT') {
        logger.warn('[ErrorBoundary] Network error, will retry', { code: error.code });
  ...
  async wrapCommand(commandName, executionFn, context = {}) {
    ...
    try {
      ...
      if (this.isCircuitBroken(commandName)) {
        throw new Error(`Circuit breaker open for ${commandName}. Command temporarily disabled.`);
      }
      ...
      if (duration > 5000) {
        logger.warn(`[ErrorBoundary] Slow command detected: ${commandName} took ${duration}ms`);
      }
      ...
      try {
        await db.db.run(
          `INSERT INTO error_logs (command, error_message, stack_trace, context, timestamp, recovery_attempted)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [commandName, error.message, error.stack, JSON.stringify(context), Date.now(), false]
        );
      } catch (dbError) {
        logger.error('[ErrorBoundary] Failed to log error to database', dbError);
      }
      ...
      if (recovery.recovered && recovery.retry) {
        ...
        try {
          ...
          logger.error(`[ErrorBoundary] Retry failed for ${commandName}`, retryError);
        }
      }
  ...
  async attemptRecovery(error, context) {
    ...
    for (const [pattern, strategy] of this.recoveryStrategies.entries()) {
      if (errorString.includes(pattern) || error.code?.toString().includes(pattern)) {
        try {
          const result = await strategy(error, context);
          if (result.recovered) {
            logger.info('[ErrorBoundary] Error recovered using strategy', { pattern });
            return result;
          }
        } catch (recoveryError) {
          logger.error('[ErrorBoundary] Recovery strategy failed', { pattern, recoveryError });
        }
      }
    }
  ...
  recordFailure(feature) {
    const breaker = this.circuitBreakers.get(feature) || { failures: 0, state: 'closed' };
    breaker.failures++;
    breaker.lastFailure = Date.now();
    
    if (breaker.failures >= this.CIRCUIT_BREAK_THRESHOLD) {
      breaker.state = 'open';
      logger.warn(`[ErrorBoundary] Circuit breaker opened for ${feature} after ${breaker.failures} failures`);
  ...
  startErrorMonitoring() {
    ...
    setInterval(() => {
      ...
      for (const [command, errorData] of this.lastErrors.entries()) {
        if (now - errorData.timestamp < 300000) { // Last 5 minutes
  ...
  getStats() {
    const stats = {
      ...
      circuitBreakers: Object.fromEntries(
        Array.from(this.circuitBreakers.entries()).map(([key, val]) => [key, val.state])
      ),
      recentErrors: Array.from(this.lastErrors.entries()).map(([cmd, data]) => ({
        command: cmd,
        count: data.count,
    ...
    try {
      ...
      for (const adminId of adminIds) {
        try {
          ...
          await user.send(`🚨 **System Alert**\n${message}\n\nTimestamp: <t:${Math.floor(Date.now() / 1000)}:F>`);
        } catch (err) {
          logger.error('[ErrorBoundary] Failed to notify admin', { adminId, error: err.message });
        }
      }
    } catch (error) {
